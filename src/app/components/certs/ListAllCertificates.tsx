...
const ListAllCertificates = () => {
  ...
  return (
    <div>
      ...
      />
      <div>
        Total Records: {data?.length || 0} | Filtered Records: {data?.filter(cert => cert.primaryDomain.includes(searchText) || (cert.otherDomains as {domain: string}[]).some(domain => domain.domain.includes(searchText)) || cert.status.includes(searchText) || cert.server?.name.includes(searchText) || (cert.requestedBy?.name || '').includes(searchText)).length || 0}
      </div>
